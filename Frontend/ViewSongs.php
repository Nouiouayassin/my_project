...
$id = $_REQUEST['id'];
$getAlbumImage = "SELECT * FROM tblalbum WHERE id = '$id'";
$resul1 = $conn->query($getAlbumImage);
        
        $resul1->setFetchMode(PDO::FETCH_ASSOC);
while($rowImage = $resul1->fetch()){
...
jQuery(document).ready(function() {
	...
	var audioPlaylist = new Playlist("1", [
		<?php
		$id = $_REQUEST['id'];
		$getSong = "SELECT songfile FROM tblsongs WHERE songalbum = '$id' LIMIT 8";
		$resul12 = $conn->query($getSong);
        
        $resul12->setFetchMode(PDO::FETCH_ASSOC);
		while($rowSong =$resul12->fetch()){
	...
	<div class="playlist_wrapper">
        	...
        	<div class="playlist_info">
				...
				$sql = "SELECT 
									   ...
									   AND tblalbum.id = '".$id."'";
									   $resul123 = $conn->query($sql);
        
									   $resul123->setFetchMode(PDO::FETCH_ASSOC);

				while($row = $resul123->fetch() ){
				?>
                	<tr>

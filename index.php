        ...
        <div class="vote_container"> 
        	<form id="vote" name="vote" method="post"  action="vote.php">
            	<div class="header_vote"> 
					...
					$getVote =" SELECT * FROM tblvotes";
                    $resul1 = $conn->query($getVote);
...
<div class="col1"><!--Start Bottom web content-->
   	 <div class="header">
            ...
            <?php 
			require_once('Administrator/PHP/connect.php');
			$sql = "SELECT * FROM tblalbum ORDER BY id DESC LIMIT 7";
            $resul2 = $conn->query($sql);
        
                    $resul2->setFetchMode(PDO::FETCH_ASSOC);
			while($rowAlbum = $resul2->fetch() ){
			?>
    	...
    	<div class="header">
            ...
            <table cellpadding="0" cellspacing="0" height="230" id="table"> 
				...
				<?php 
				$getVotes = "SELECT songfile,songpoints FROM tblsongs WHERE songpoints >=1 ORDER BY songpoints DESC LIMIT 6" ;
                $resul12 = $conn->query($getVotes);
        
                    $resul12->setFetchMode(PDO::FETCH_ASSOC);
                
				while($row = $resul12->Fetch()){
				$r = rand(128,255);
				$g = rand(128,255);
				$b = rand(128,255);
     ...
     <div class="header">
    	...
    	<div class="header">
            ...
            <?php 
			$sql = "SELECT tblsongs.id,tblsongs.songfile,tblalbum.albumimage,tblalbum.albumname,tblsongs.songsinger FROM tblsongs,tblalbum WHERE tblsongs.songalbum = tblalbum.id AND songcat = 'OPM' ORDER BY id DESC LIMIT 7";
            $resul132 = $conn->query($sql);
        
                    $resul132->setFetchMode(PDO::FETCH_ASSOC);
			while($rowAlbum = $resul132->Fetch()){
			?>
            <div class="album_holder">
            	<div class="content_holder">
                	<div class="content">
